---
title: "OppsHub"
description: "Herramienta de gestión de proyectos construida desde cero. Kanban, milestones, finanzas, equipo y seguimiento de hábitos en una sola app."
description_en: "Project management tool built from scratch. Kanban, milestones, finances, team and habit tracking in one app."
role: "Creador & Desarrollador"
role_en: "Creator & Developer"
organization: "Proyecto personal"
organization_en: "Personal project"
period: "2026"
status: "ongoing"
tags: ["React", "Supabase", "Full-stack", "SaaS", "Tailwind"]
outcomes:
  - "Gestión completa de proyectos: Kanban drag & drop, milestones e hitos"
  - "Módulo de finanzas y reportes por proyecto con visualización de datos"
  - "Autenticación, roles de equipo, notificaciones y log de actividad"
outcomes_en:
  - "Full project management: drag & drop Kanban, milestones and checkpoints"
  - "Finance module and per-project reports with data visualization"
  - "Authentication, team roles, notifications and activity log"
techStack: ["React 19", "Vite", "Supabase", "Tailwind v4", "dnd-kit", "Recharts"]
featured: true
order: 1
caseStudyAvailable: true
liveUrl: "https://opps-hub.vercel.app/"
previewVideo: "/Oppshubvid.mp4"
---

## Qué es

OppsHub es una herramienta de gestión de proyectos que construí para cubrir un caso de uso que no encontraba bien resuelto en las herramientas existentes: coordinar proyectos pequeños con seguimiento de finanzas, hábitos de equipo y reportes en un mismo sitio.

No es una demo. Es una app funcional con autenticación, base de datos propia y lógica de negocio real.

## Qué tiene

- **Kanban** con drag & drop nativo (dnd-kit), estados personalizables y detalle de tarea completo
- **Milestones** con progreso y fechas de entrega
- **Finanzas** por proyecto: ingresos, gastos y balance visual
- **Equipo**: invitación por enlace, roles y notificaciones
- **Hábitos**: seguimiento de rutinas de equipo ligadas al proyecto
- **Reportes**: gráficas con Recharts y exportación

## Stack

React 19 + Vite en el frontend, Supabase como backend (auth, base de datos PostgreSQL y storage). Todo el estado de servidor se gestiona directamente contra Supabase sin capa intermedia.

## Por qué lo construí

Quería entender cómo se construye una SaaS de principio a fin — desde el modelo de datos hasta la experiencia de usuario. OppsHub es el resultado de eso.
